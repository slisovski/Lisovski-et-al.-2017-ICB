...
# plot(raster(tempA_MNT[,,3]))

month.ind <- seq(as.POSIXct("1948-01-01"), as.POSIXct("2017-02-01"), by = "month")
...
temp.s  <- list(Amp = array(dim = c(dim(tempA_MNT)[1], dim(tempA_MNT)[2], 11)),
               Pre = array(dim = c(dim(tempA_MNT)[1], dim(tempA_MNT)[2], 11)),
               wt  = array(dim = c(dim(tempA_MNT)[1], dim(tempA_MNT)[2], 2)))
# load("Revision_Analysis/Output/Temperature/temp_monthly_seasonality.RData")

for(r in 1:dim(temp.s$Amp)[1]) {
  for(c in 1:dim(temp.s$Amp)[2]) {
    
    cat(paste(rep("\b\b\b\b\b\b", 6), collapse = ""))
    cat(r, " - ", c, "\r")
    
    y <- tempA_MNT[25, 75, format(month.ind, "%Y")%in%c(2006:2015)]
    # plot(y, type = "o")
    
    if(all(is.na(temp.s$Amp[r,c,])) & (!all(is.na(y)) & abs(diff(quantile(y, probs = c(0.9, 0.1), na.rm = T)))>0.25)) { 
      ...
      if(!is.null(wt.out) & (any(wt.out[,1]>0.85 & wt.out[,1]<1.15) | any(wt.out[,1]>0.45 & wt.out[,1]<0.65))) {
        ...
        temp.s$Amp[r,c,fcy[,1]] <- fcy[,2]
        temp.s$Pre[r,c,fcy[,1]] <- ifelse(fcy[,3]<0, 0, fcy[,3])
        temp.s$wt[r,c,1] <- wt.out[1,1]
        temp.s$wt[r,c,2] <- wt.out[1,2]
  ...
  if(floor(r/5)==r/5) {
    ...
    plot(wrld_simpl, add = T)
    plot(raster(temp.s$wt[,,2], 1:2, xmn = -180, xmx = 180, ymn = -90, ymx = 90))
    plot(wrld_simpl, add = T)
    par(opar)
    dev.off()
    
    save(temp.s, file = "Revision_Analysis/Output/Temperature/temp_monthly_seasonality.RData")
  }
} # end r
