...
### Date: March 2017                                                       ##
#############################################################################
### Seasonal Components: Monthly mean temperatures (1x1 degree grid)       ##
#############################################################################
...
prec.s  <- list(Amp = array(dim = c(dim(precA_MNT)[1], dim(precA_MNT)[2], 10)),
                Pre = array(dim = c(dim(precA_MNT)[1], dim(precA_MNT)[2], 10)),
                wt  = array(dim = c(dim(precA_MNT)[1], dim(precA_MNT)[2], 2)))
# load("Revision_Analysis/Output/Precipitation/prec_monthly_seasonality.RData")

for(r in 1:dim(prec.s$Amp)[1]) {
  for(c in 1:dim(prec.s$Amp)[2]) {
    
    cat(paste(rep("\b\b\b\b\b\b", 6), collapse = ""))
    cat(r, " - ", c, "\r")
    #r = 20; c = 70
    
    y <- precA_MNT[r, c, format(month.ind, "%Y")%in%c(2006:2015)]
    # plot(y, type = "o")
    
    if(all(is.na(prec.s$Amp[r,c,])) & (!all(is.na(y)) & abs(diff(quantile(y, probs = c(0.9, 0.1), na.rm = T)))>0.05) & !is.na(as.matrix(land.r[r,c]))) { 
      
      ### Wavelet analysis to determine significant periodicity
      wt  <- tryCatch(wt(cbind(1:length(y),y)), error = function(e) paste("Error"))
      if(!is.character(wt)) {
      ...
      if(!is.null(wt.out) & (any(wt.out[,1]>0.85 & wt.out[,1]<1.15) | any(wt.out[,1]>0.45 & wt.out[,1]<0.65))) {
        ...
        dat <- data.frame(t = 1:length(y), 
                          years = as.numeric(format(month.ind, "%Y"))[format(month.ind, "%Y")%in%c(2006:2015)], 
                          y = y,
                          y.norm = scale(y))
        ...
        # plot(month.ind[format(month.ind, "%Y")%in%c(2006:2015)], y, type = "o")
        #   lines(month.ind[format(month.ind, "%Y")%in%c(2006:2015)], curve.1, lwd = 2, col = "orange")
        ...
        prec.s$Amp[r,c,fcy[,1]] <- fcy[,2]
        prec.s$Pre[r,c,fcy[,1]] <- ifelse(fcy[,3]<0, 0, fcy[,3])
        prec.s$wt[r,c,1] <- wt.out[1,1]
        prec.s$wt[r,c,2] <- wt.out[1,2]
      } 
      }
    }
  }
  ...
  if(floor(r/5)==r/5) {
    ...
    plot(wrld_simpl, add = T)
    plot(raster(prec.s$wt[,,2], 1:2, xmn = -180, xmx = 180, ymn = -90, ymx = 90))
    plot(wrld_simpl, add = T)
    par(opar)
    dev.off()
    
    save(prec.s, file = "Revision_Analysis/Output/Precipitation/prec_monthly_seasonality.RData")
  }
} # end r
